...
bool Mesh::loadObj(const char* filename, bool loadTextures /* = true */, bool flipTextureV /* = false */) {
	...
	for (auto& m : materials) {
		...
		// textures
		m_materials[index].alphaTexture.loadTexture((folder + m.alpha_texname).c_str());
		m_materials[index].ambientTexture.loadTexture((folder + m.ambient_texname).c_str());
		m_materials[index].diffuseTexture.loadTexture((folder + m.diffuse_texname).c_str());
		m_materials[index].specularTexture.loadTexture((folder + m.specular_texname).c_str());
		m_materials[index].specularHighlightTexture.loadTexture((folder + m.specular_highlight_texname).c_str());
		m_materials[index].normalTexture.loadTexture((folder + m.bump_texname).c_str());
		m_materials[index].displacementTexture.loadTexture((folder + m.displacement_texname).c_str());
	...
	for (auto& s : shapes) {
		...
		// fill vertex buffer
		glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(AIEVertex), vertices.data(), GL_STATIC_DRAW);
		...
		glVertexAttribPointer(2, 4, GL_FLOAT, GL_TRUE, sizeof(AIEVertex), (void*)(sizeof(glm::vec4) * 2));

		// enable texture coords
		glEnableVertexAttribArray(3);
		glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(AIEVertex), (void*)(sizeof(glm::vec4)*3));
...
void Mesh::draw(unsigned int primitiveType) {
	...
	for (auto& c : m_meshChunks) {
		...
		if (currentMaterial != c.materialID) {
			...
			if (m_materials[currentMaterial].diffuseTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].diffuseTexture.handle);
			else if (diffuseTexUniform >= 0)
				glBindTexture(GL_TEXTURE_2D, 0);

			glActiveTexture(GL_TEXTURE1);
			if (m_materials[currentMaterial].alphaTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].alphaTexture.handle);
			else if (alphaTexUniform >= 0)
				glBindTexture(GL_TEXTURE_2D, 0);

			glActiveTexture(GL_TEXTURE2);
			if (m_materials[currentMaterial].ambientTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].ambientTexture.handle);
			else if (ambientTexUniform >= 0)
				glBindTexture(GL_TEXTURE_2D, 0);

			glActiveTexture(GL_TEXTURE3);
			if (m_materials[currentMaterial].specularTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].specularTexture.handle);
			else if (specTexUniform >= 0)
				glBindTexture(GL_TEXTURE_2D, 0);

			glActiveTexture(GL_TEXTURE4);
			if (m_materials[currentMaterial].specularHighlightTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].specularHighlightTexture.handle);
			else if (specHighlightTexUniform >= 0)
				glBindTexture(GL_TEXTURE_2D, 0);

			glActiveTexture(GL_TEXTURE5);
			if (m_materials[currentMaterial].normalTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].normalTexture.handle);
			else if (normalTexUniform >= 0)
				glBindTexture(GL_TEXTURE_2D, 0);

			glActiveTexture(GL_TEXTURE6);
			if (m_materials[currentMaterial].displacementTexture.handle > 0)
				glBindTexture(GL_TEXTURE_2D, m_materials[currentMaterial].displacementTexture.handle);
...
bool AIETexture::loadTexture(const char* path) {
...
void Mesh::calculateTangents(std::vector<AIEVertex>& vertices, const std::vector<unsigned int>& indices) {
	...
	for (unsigned int a = 0; a < vertexCount; a++) {
		...
		// calculate bitangent (ignoring for AIEVertex)
