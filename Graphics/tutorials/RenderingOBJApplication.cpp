...
RenderingOBJApplication::RenderingOBJApplication()
	: m_camera(nullptr),
	m_shader(nullptr),
	m_mesh(nullptr) {
...
bool RenderingOBJApplication::startup() {
	...
	// load a mesh
	m_mesh = new Mesh();
	if (m_mesh->loadObj("./models/stanford/dragon.obj") == false)
		return false;

	// load a shader
	m_shader = new Shader();
	if (m_shader->loadShader(GL_VERTEX_SHADER, "./shaders/flat.vert") == false) {
		printf("Vertex Shader Error: %s\n", m_shader->getLastError());
		return false;
	}
	if (m_shader->loadShader(GL_FRAGMENT_SHADER, "./shaders/flat.frag") == false) {
...
void RenderingOBJApplication::shutdown() {
...
bool RenderingOBJApplication::update(float deltaTime) {
...
void RenderingOBJApplication::draw() {
